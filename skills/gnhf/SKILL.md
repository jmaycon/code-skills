...
### Native Antigravity Subagents (Alternative)

When the host agent is Antigravity (AGY) and the `gnhf` CLI tool fails to spawn the agent headlessly (e.g., `--agent acp:agy` causing rapid crash loops), bypass the CLI and emulate GNHF natively:

1. Use the `invoke_subagent` tool to spawn one or more native subagents (e.g. `self`).
2. Inject the GNHF constraints directly into the subagent's prompt. The prompt must command them to act as a long-running GNHF worker: working slice-by-slice, verifying each slice, leaving notes if blocked, and stopping **only** when the specific observable condition is met.
3. Act as the Companion orchestrator by monitoring the subagents' messages and steering them if they drift.
...
- `acp:<target>`: explicit ACP target request, or when the user wants to drive a custom ACP-compatible agent through GNHF. (Note: `acp:agy` may fail if the Antigravity CLI isn't configured for headless stdio ACP. If this happens, use Native Antigravity Subagents instead.)
